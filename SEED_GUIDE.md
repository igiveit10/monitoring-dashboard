...
## ⚠️ 중요: 운영 환경에서 FORCE_SEED 사용 금지

**운영 환경(Render 프로덕션)에서는 절대 `FORCE_SEED=true`를 사용하지 마세요.**

- `FORCE_SEED=true`는 모든 기존 데이터를 삭제하고 재시드합니다
- 운영 데이터가 영구적으로 삭제될 수 있습니다
- `FORCE_SEED`는 로컬 개발 환경에서만 사용하세요
